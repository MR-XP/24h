...
function success($data) {
    return result(Code::SUCCESS, Code::$codes[Code::SUCCESS], $data);
}

//返回错误
function error($code = 0, $message = '', $data = []) {
    if ($code != 0) {
        return result($code, Code::$codes[$code], $data);
    } else {
...
//返回列表数据
function page_result($list, $pageNo, $pageSize, $totalResults, $totalPages) {
    return success([
        'list' => $list,
        'page_no' => $pageNo,
        'page_size' => $pageSize,
