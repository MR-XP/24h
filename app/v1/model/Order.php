        ...
        if ($data['type'] == self::TYPE_BUY_CARD) { //购卡
            ...
            if($product['max_buy'] != 0){
                $buyNum = self::where([
                    ...
                    //'pay_status'    => self::STATUS_ENABLE
                ])->count();
                if($buyNum >= $product['max_buy']){
                    return error(Code::VALIDATE_ERROR, '该会员卡最多购买' . $product['max_buy'] . '次，请确认下自己是否已经买过或有未支付的订单');
                }
            }
            ...
            if(empty($product['price']) || $product['price'] == 0 || $product['price'] < 2){     //没有价格，默认为200元
                $data['price']          = 200;  
                $data['sale_price']     = 200; 
                $data['origin_price']   = 200;
            }else{
...
//                $orderModel=new Order();
    ...
    /**
     * @desc 检查会员订单是否有参与活动的会员卡
     * @author lihongqiang
     * @return int|string
     */
    public function checkOrderHasActivity($user_id)
    {
        $where = [];
        $where['a.type'] = self::TYPE_BUY_CARD; //会员卡
        $where['a.pay_status'] = self::PAY_STATUS_COMPLETED;
        $where['a.user_id'] = $user_id;
        $where['b.is_activity'] = \app\common\model\Card::IS_ACTIVITY; //有参与活动暂定为世界杯
        $count = $this->alias('a')->join('sys_card b', 'a.product_id=b.card_id')->where($where)->count();
        return $count;
    }
}
