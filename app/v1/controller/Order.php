...
//            $result = $Pay->wxpay($order_id, $this->member['open_id'], 0);
     ...
     * @return array
     */
    public function orderHasActivity()
    {
        $user_id = $this->user['user_id'];
        $user_id = intval($user_id);
        $Order = new model\Order();
        $activityCount = $Order->checkOrderHasActivity($user_id);
        if ($activityCount) {
            return success('有活动卡，可以去竞猜');
        }
        else{
            return error('0','没有购买活动卡，无法竞猜');
        }
    }
}
