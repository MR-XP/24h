...
class YunTongXun {
    ...
    function __construct($config) {
        ...
        $this->enabeLog = $config['enabe_log'];
        $this->LogPath = $config['log_path'];
        $this->AccountSid = $config['account_sid'];
        $this->AccountToken = $config['auth_token'];
        $this->AppId = $config['app_id'];
        $this->DefaultTplId = $config['default_tpl_id'];
        if (!is_dir($this->LogPath)) {
            mkdir($this->LogPath);
        }
        $this->Handle = fopen($this->LogPath . 'log.txt', 'a');
    }

    /**
     * 发送短信
     * @param type $phone 手机
     * @param array $datas 替换的模板内容，数组
     * @param type $tplId 模板ID
     * @return type
     */
    function sendSms($phone, $datas, $tplId = 0) {
        $tplId == 0 && $tplId = $this->DefaultTplId;
        $result = $this->sendTemplateSMS($phone, $datas, $tplId);
        if ($result == null) {
            return error('YunTongXun result error!');
        }
        if ($result->statusCode != 0) {
            return error(\app\common\component\Code::VALIDATE_ERROR, $result->statusMsg);
        } else {
            return success($result->TemplateSMS);
        }
    }
