...
class WxPayDataBase {
     ...
     * @param string $value 
     * */
    public function SetSign($wxPayAppKey) {
        $sign = $this->MakeSign($wxPayAppKey);
     ...
     */
    public function MakeSign($wxPayAppKey) {
        //签名步骤一：按字典序排序参数
        ksort($this->values);
        $string = $this->ToUrlParams();
        //签名步骤二：在string后加入KEY
        $string = $string . "&key=" . $wxPayAppKey;
        //签名步骤三：MD5加密
        $string = md5($string);
        //签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase {
     ...
     * 检测签名
     */
    public function CheckSign($wxPayAppKey) {
        //fix异常
        if (!$this->IsSignSet()) {
     ...
     * @throws WxPayException
     */
    public static function Init($xml,$wxPayAppKey) {
        $obj = new self();
        $obj->FromXml($xml);
        //fix bug 2015-06-29
