...
class WxPayApi {
     ...
     * @return 成功时返回，其他抛异常
     */
    public static function unifiedOrder($inputObj, $appId, $appSecret, $wxPayMchId, $wxPayAppKey, $gatewayUrl, $timeOut = 6) {
        //$url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
        $url = $gatewayUrl . '/pay/unifiedorder';
        //检测必填参数
...
//		}
        ...
        //签名
        $inputObj->SetSign($wxPayAppKey);
        $xml = $inputObj->ToXml();

        $startTimeStamp = self::getMillisecond(); //请求开始时间
        $response = self::postXmlCurl($xml, $url, false, $timeOut);
        $result = WxPayResults::Init($response, $wxPayAppKey);
     ...
     * @return 成功时返回，其他抛异常
     */
    public static function orderQuery($appId, $wxPayMchId, $wxPayAppKey, $inputObj, $timeOut = 6) {
        ...
        $inputObj->SetMch_id($wxPayMchId); //商户号
        $inputObj->SetNonce_str(self::getNonceStr()); //随机字符串

        $inputObj->SetSign($wxPayAppKey); //签名
        $xml = $inputObj->ToXml();
     ...
     */
    public static function notify($callback, &$msg, $wxPayAppKey) {
        //获取通知的数据
        //$xml = $GLOBALS['HTTP_RAW_POST_DATA'];
        $xml = file_get_contents('php://input');
        //如果返回成功则验证签名
        try {
            $result = WxPayResults::Init($xml, $wxPayAppKey);
