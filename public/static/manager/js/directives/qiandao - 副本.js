        ...
        for (w = 1; w < 7; w++) {
            ...
            for (d = 0; d < 7; d++) {
                ...
                htmls.push("<div><span class='on' onclick='showOtherDiv($(this).text())'>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</span></div>");
                    ...
                    htmls.push("<div onclick='showOtherDiv($(this).text())'>" + (!isNaN(myMonth[w][d]) ? myMonth[w][d] : " ") + "</div>");
                }
            }
            htmls.push("</div>");
        }
...
function showOtherDiv(num){
    ...
    $.ajax({
        ...
        data:{
            ...
            page_no     :   1,
            page_size   :   100,
            start_time  :   signTimeS,
            end_time    :   signTimeE,
        },
        success:function(data){
            if(data.code==200){
                sign_list = data.data.list;
                ...
                for(var i=0;i<sign_list.length;i++){
                    var time_stampS =new Date(sign_list[i].sign_time.replace(/-/gi,'/'));//开始时间戳
                    var time_stampE =new Date(sign_list[i].out_time.replace(/-/gi,'/'));//开始时间戳
                    console.log(time_stampS);
                    console.log(time_stampE);

                    if(sign_list[i].out_time=='0000-00-00 00:00:00'){
                        $(".sign_content").append(
                            "<div class='col-md-4 col-xs-4 text-center' >"+sign_list[i].sign_time.substring(10,20)+"</div>"+
                            "<div class='col-md-4 col-xs-4 text-center' >"+sign_list[i].out_time.substring(10,20)+"</div>"+
                            "<div class='col-md-4 col-xs-4 text-center' >"+"未扫离场码"+"</div>"
                        );
                    }else{
                        var time_difference = (time_stampE-time_stampS)/1000/60;
                        console.log(Math.round(time_difference));
                        $(".sign_content").append(
                            "<div class='col-md-4 col-xs-4 text-center' >"+sign_list[i].sign_time.substring(10,20)+"</div>"+
                            "<div class='col-md-4 col-xs-4 text-center' >"+sign_list[i].out_time.substring(10,20)+"</div>"+
