...
app.controller('statisticaBoundController', ['$scope', '$http', '$state','$location','$filter','$stateParams','infoService','formatDate','arrDate',
	...
	$scope.load=function(){
		...
		$http({
            ...
            if(data.code==200){
                $scope.statisticaBoundList=data.data;
                if($scope.statisticaBoundList.list.length == 'undefined' || $scope.statisticaBoundList.list.length == undefined){
                	$scope.statisticaBoundList.list = [];
                }
            }else{
                layer.msg(data.message, {icon: 2});
            }
