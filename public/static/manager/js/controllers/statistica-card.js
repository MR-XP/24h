...
app.controller('statisticaCardController', ['$scope', '$http', '$state','$location','$filter','$stateParams','infoService','formatDate','arrDate',
	...
	$scope.load=function(){
		...
		$http({
            ...
            if(data.code==200){
                $scope.statisticaCardList=data.data;
                if($scope.statisticaCardList.list.length == 'undefined' || $scope.statisticaCardList.list.length == undefined){
                	$scope.statisticaCardList.list = [];
                }
            }else{
                layer.msg(data.message, {icon: 2});
            }
