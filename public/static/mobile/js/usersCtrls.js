	...
	function($scope, $ionicLoading, ajaxService, $ionicScrollDelegate, $timeout, $ionicActionSheet, $popup, $location) {
		...
		$scope.toggleActive = function(v) {
			...
			if(v == 0)
				...
				angular.element('.tab-nav-look').css({
					'left': '15%',
					'margin-left': angular.element('.tab-nav-look').width() + 2 + 'px'
				});
		...
		$scope.cmet = function(item) {
			...
			var cmet = $ionicPopup.prompt({
				...
				if(res) {
					...
					ajaxService.ajax('courseComment', {
...
app.controller("cardListCtrl", ["$scope", "$ionicLoading", "ajaxService", "$ionicScrollDelegate", "$ionicPopup", "$location",
	function($scope, $ionicLoading, ajaxService, $ionicScrollDelegate, $ionicPopup, $location) {
		...
		$scope.loadActivity = function() {
			...
			var myYear    = nowDate.getFullYear();    //获取完整的年份(4位,1970-????)
			var myMonth   = nowDate.getMonth()+1;       //获取当前月份(0-11,0代表1月)
			var myData    = nowDate.getDate();        //获取当前日(1-31)
			var myHours   = nowDate.getHours();       //获取当前小时数(0-23)
			var myMinutes = nowDate.getMinutes();     //获取当前分钟数(0-59)
			var mySeconds = nowDate.getSeconds();     //获取当前秒数(0-59)
			myMonth       = (myMonth<10 ? "0"+myMonth:myMonth);
			myData        = (myData<10 ? "0"+myData:myData);
			...
			var nowTime = myYear+'-'+myMonth+'-'+myData+' '+myHours+':'+myMinutes+':'+mySeconds;
			ajaxService.ajax('getCurrentActivity', {
				time     :    nowTime,     //当前时间
			}, function(json) {
				if(json.code == 200) {
					$scope.activityData = json.data[0];
		...
		$scope.goQuiz = function() {
			ajaxService.ajax('orderHasActivity', {
			}, function(json) {
				if(json.code == 0){
					var a = $ionicPopup.confirm({
						title: '尊敬的用户',
						template: '您还没有购买活动卡,无法竞猜,是否立即去购买',
						cancelText: '不用了',
						okText: '去购买',
						cssClass: 'tc'
					});
					a.then(function(res) {
						if(res) {
							$location.path('users/card-dtl/' + $scope.activityData.card_id);
						}
					})
				}
				if(json.code == 200) {
					$location.path('users/quiz');
				}
			})
		}
...
//会员卡详情
app.controller("cardDtlCtrl", ["$scope", "$ionicLoading", "ajaxService", "$stateParams", "$location","$popup",
	function($scope, $ionicLoading, ajaxService, $stateParams, $location,$popup) {
	...
	function($scope, $ionicLoading, ajaxService, $stateParams, $filter, formatDate, infoService, $location, $ionicPopup) {
		...
		$scope.go = function() {
			...
			if(!window.member) {
				...
				$scope.coachItem.shop_id = $scope.info.shop_id;
			...
			if(!window.member) {
				...
				ajaxService.ajax('make', {
					...
					if(json.code == 200) {
						...
						} else if(json.code == 0 && json.message.substr(0, 7) == '你累计旷课已达') {
						...
						} else if(json.code == 0 && json.message == '') {

							$scope.go('您没有可用的会员卡，请先购买哟。', 1, json)
				...
				a.then(function(res) {
					...
					if(res) {
						...
						if(type == 1) {
							...
							$scope.info.course.shop_id = $scope.info.coach.shop_id;
...
app.controller("myCardCtrl", ["$scope", "$ionicLoading", "ajaxService", "$stateParams", "$ionicPopup", "$popup", "$location",
	function($scope, $ionicLoading, ajaxService, $stateParams, $ionicPopup, $popup, $location) {
		...
		$scope.getIpt = function() {
			...
			nowUse = $scope.cardInfo.other_users.length + angular.element('.setipt').length;
			maxUse = $scope.cardInfo.max_use;
			addUse = $scope.cardInfo.add_use;

			if(nowUse >= (maxUse + addUse)) {
				if($scope.cardInfo.addMbr_status == 0) {
					var a = $ionicPopup.confirm({

						title: '尊敬的用户',
						template: '您的会员卡家庭成员已达到上限，是否增加人数？（增加一名按购买会员卡金额的40%收取）',
						cancelText: '不用了',
						okText: '立即前往',
						cssClass: 'tc'
					});
				...
				if(nowUse < maxUse) {
					angular.element('.add-ipt').before(
						'<div><input class="setipt" type="tel" maxlength="11" free="0" /></div>'
					)
				}

				if(nowUse >= maxUse && nowUse < (maxUse + addUse)) {
					angular.element('.add-ipt').before(
						'<div><input class="setipt" type="tel" maxlength="11" free="1" /></div>'
					)
				}
		...
		$scope.member = function() {
			...
			for(var i = 0; i < angular.element('.setipt').length; i++) {
				...
				phones.push({
					phone: angular.element('.setipt:eq(' + i + ')').val(),
					free: angular.element('.setipt:eq(' + i + ')').attr('free'),
				})
		...
		$scope.$on('$ionicView.beforeEnter', function() {
			//$ionicLoading.show(HX_CONFIG.loadingBase);
			$scope.load();
			if(!window.shop.id || window.shop.id == 0){
...
app.controller("memberCardCtrl", ["$scope", "$ionicLoading", "ajaxService", "$stateParams","$popup",
	function($scope, $ionicLoading, ajaxService, $stateParams,$popup) {
		...
		$scope.load = function() {
			...
			ajaxService.ajax('memberCard', {
				...
				if(json.code == 200) {
					...
					setTimeout(function() {
						for(var i=0;i<$scope.list.length;i++){
							$('.switch').eq(i).attr('id','switch'+$scope.list[i].user_id);
							// $('.switch-lable').eq(i).attr('for','switch'+$scope.list[i].user_id);
							// $('.slideThree').eq(i).attr('name','switch'+$scope.list[i].user_id)
							$('.switch').eq(i).attr("key", i);
							if($scope.list[i].status == 0){
								$('.switch').eq(i).attr("checked", true);
								// $('.slideThree').eq(i).css('background-color','#ff9d35');
							}
							if($scope.list[i].status == 1){
								$('.switch').eq(i).attr("checked", false);
								// $('.slideThree').eq(i).css('background-color','#eaeaea');
							}
		...
		$scope.manage = function(){
			if($scope.lockStatus == false){
				$scope.lockStatus = true;
			}else{
				$scope.lockStatus = false;
			}
		}
		$scope.lock = function(id_s,status,id_u){
			var key = $('.switch[id = switch'+id_u+']').attr('key');
			$scope.usersArr = [];
			$scope.usersArr.push(id_u);
			if(status == 0){
				$scope.statusNum = 1;
			}else{
				$scope.statusNum = 0;
			...
			ajaxService.ajax('lockMemberCard', {

				sold_card_id   :   id_s,             //string 用户的卡id
				users          :   $scope.usersArr,  //array 需要操作的user_id
				status         :   $scope.statusNum, //string 状态，0为解锁，1为锁定

			}, function(json) {

				if(json.code == 200) {
					
					if($scope.statusNum == 1){
						// $('.slideThree[name = switch'+id_u+']').css('background-color','#eaeaea');
						$scope.list[key].status = 1;
					}
					if($scope.statusNum == 0){
						// $('.slideThree[name = switch'+id_u+']').css('background-color','#ff9d35');
						$scope.list[key].status = 0;
					}
					// $scope.load();
					...
					var checked;
					status == 0?checked = true : checked=false;
					$('input[id=switch'+id_u+']').prop('checked',checked);
					$popup.alert('尊敬的用户', null, json.message);
...
//竞猜页面
app.controller("quizCtrl", ["$scope", "$ionicLoading", "ajaxService", "$stateParams","$ionicPopup","$timeout","$location",
	function($scope, $ionicLoading, ajaxService, $stateParams,$ionicPopup,$timeout,$location) {
		//动态设置高度
		var num = Math.ceil( $('.list .team').length / 4);
		var teamHeight = (200*num+10)/20;
		$('.team-list .list').css("height",teamHeight+"rem");
		...
		$scope.loadActivity = function() {
			ajaxService.ajax('getCurrentActivity', {
				...
				if(json.code == 200) {
					$scope.activityData = json.data;
					var dataArr = $scope.activityData[0].data.split(','); //以分号拆分字符串
					$scope.activityTeam = [];

					for(var i=0;i<dataArr.length;i++){
						ajaxService.ajax('getActivityTeam', {
							team_id     :       dataArr[i], //球队id
						}, function(json) {
							if(json.code == 200) {
								$scope.activityTeam.push(json.data[0]);
							}
						})
					}
				}
		...
		$scope.chooseTeam = function(id) {
			var Alpha = $('.team-logo[value = '+id+']').css("opacity");
			if(Alpha < 1){
				$('.team-logo[value = '+id+']').css("opacity","1");
			}else{
				$('.team-logo[value = '+id+']').css("opacity","0.4");
			}
		}
		
		$scope.quizSave = function() {
			var teamCheck = [];
			for(var i=0;i<$('.team-logo').length;i++){
				var teamStatus = $('.team-logo').eq(i).css("opacity");
				if(teamStatus == 1){
					teamCheck.push($('.team-logo').eq(i).attr("value"));
				}
			}
			if(teamCheck.length<($scope.activityTeam.length/2)){
				var myPopup = $ionicPopup.show({
          title: '您选择的球队太少了,请选择16个球队',
        });
        $timeout(function() {
        ...
        $timeout(function() {
          myPopup.close(); // 3秒后关闭弹窗
			...
			var teamStr = '';
			for(var i=0;i<teamCheck.length;i++){
				if(i==teamCheck.length-1){
					teamStr = teamStr + teamCheck[i];
				}else{
					teamStr = teamStr + teamCheck[i]+',';
				}
			}
			ajaxService.ajax('checkOnlyActivity', {
				type    :    $scope.activityTeam.length/2,
			}, function(json) {
				if(json.code == 200) {
					ajaxService.ajax('activitySave', {
						id     : 0,         //0是新增；非0是编辑
						data   : teamStr,
						values : $scope.activityTeam.length/2,
						status : 1,         //1：正常；0：失效
					}, function(json) {
						if(json.code == 200) {
							console.log(json.data);
							var myPopup = $ionicPopup.show({
			          title: '竞猜成功，请耐心等待结果通知',
			        });
			        $timeout(function() {
			          myPopup.close(); // 3秒后关闭弹窗
			        }, 1500);
