	...
	return {
		...
		link: function(scope, element, attr) {
			...
			for(var i = 0; i < 7; i++) {
				...
				html+='<a href="javascript:;" class="date-menu-li" nowDay="'+$filter('zeroFill')(f.d)+'" ng-click="'+attr.click+'(\''+ f.y + "-" + $filter('zeroFill')(f.m) + "-" + $filter('zeroFill')(f.d)+'\','+i+')">'+dayMap[day]+'</a>';
		...
		link: function($scope, element, attr, controller) {
			...
			$scope.getProvince=function(name,ix,sname){
				...
				if($('.region-nav:eq('+ix+')').attr('type')==0){
					...
					$('.region-nav').css('height','4rem').attr('type',0);
					...
					$('.region-nav:eq('+ix+')').css('height','4rem').attr('type',0);
				}
