...
//会员首页
app.controller("userHomeCtrl", ["$scope","$ionicLoading","ajaxService","formatDate","$filter","$timeout","$location",
		function ($scope,$ionicLoading,ajaxService,formatDate,$filter,$timeout,$location) {
	...
	//在场人数
	$scope.load=function(){
		$scope.loadHomeCourse();
		$scope.loadHomeCoach();
	}
	//跳轉到公共課列表
	$scope.goComCourseList=function(){
		$location.path('users/comCourse-list');
	}

	$scope.goComCourseDtl=function(id){
		$location.path('users/comCourse-dtl/'+id)
	}
	...
	//加载公共课
	$scope.loadHomeCourse=function(){
		ajaxService.ajax('planRec',{
			longitude   :      window.userLoc.longitude,   //经度
			latitude    :      window.userLoc.latitude,    //纬度
			shop_id     :      window.shop.id,             //场馆ID
			limit       :      3,                          //加载节数，不传默认两节
		},function(json){
			if(json.code==200){
				$scope.courseList=json.data;
				if($scope.courseList.length == 3){
					$scope.progressBar0 = $scope.courseList[0].user_count/$scope.courseList[0].max_count*100+'%';
					$scope.progressBar1 = $scope.courseList[1].user_count/$scope.courseList[1].max_count*100+'%';
					$scope.progressBar2 = $scope.courseList[2].user_count/$scope.courseList[2].max_count*100+'%';
				}
	...
	//加载教练
	$scope.loadHomeCoach=function(){
		ajaxService.ajax('coachRec',{
			shop_id  :    window.shop.id,   //场馆ID，不传场馆ID，默认加载全国人气最高的。教练按销售课程最多的人气进行顺序排列
			limit    :    3,                //加载的教练，不传默认加载前三名
		},function(json){
			if(json.code==200){
				$scope.coachList=json.data;
			}
		})
	}
...
//会员卡列表
app.controller("userCardListCtrl", ["$scope","$ionicLoading","ajaxService","$ionicScrollDelegate","$ionicPopup","$location",
				   	  function ($scope,$ionicLoading,ajaxService,$ionicScrollDelegate,$ionicPopup,$location) {
...
//会员卡详情
app.controller("userCardDtlCtrl", ["$scope","$ionicLoading","ajaxService","$stateParams","$location","$ionicModal",
				     function ($scope,$ionicLoading,ajaxService,$stateParams,$location,$ionicModal) {

	//会员卡详情加载
	$scope.load=function(){
		ajaxService.ajax('getcardinfo',{ card_id : $stateParams.id },function(json){
			if(json.code==200){
				$scope.cardDtl=json.data;
			}
		})	
	}
  ...
  $scope.$on('$destroy', function() {
	...
	$ionicModal.fromTemplateUrl('templates/counselor.html', {
    scope: $scope,
  ...
  $scope.openModal1 = function() {
    $scope.modal1.show();
    var counselorName = $("#buyCard-model .counselor span").attr('name');
    if(counselorName != undefined){
    	$('.list[name = '+counselorName+'] .selected').append("<img class='select_icon' src='./static/mobilev2/images/selected_icon.png' />");
    }
  }
  ...
  $scope.loadSale=function(){
	...
	$scope.selectSale = function(id,name){
		$scope.salemanID = id;
		$("#buyCard-model .counselor span").text(name);
		$("#buyCard-model .counselor span").attr('name',id);
		$(".list .selected img").remove();
		$scope.modal1.hide();
	}
	//去购买
	...
	$scope.createOrder=function(){
		ajaxService.ajax('createorder',{
			type			    :	1,	//1会员卡、2小团课、3私教课、4储值
			product_num		:	1,
			product_id		:	$scope.cardDtl.card_id,
			shop_id			  :	window.shop.id,
			seller_id		  :	$scope.salemanID,
		},function(json){
			if(json.code==200){
				window.location.href=window.base_url+'/payv2.html?order_id='+json.data.order_id;
			}
		})
	}
	//跳转注册(未注册用户)
...
//公共课列表
app.controller("userComCoursePlanCtrl", ["$scope","$ionicLoading","ajaxService","formatDate","$filter","$timeout","$location",
				   			 function ($scope,$ionicLoading,ajaxService,formatDate,$filter,$timeout,$location) {
	...
	//日期选择
	$scope.clickDateMenu=function(time,ix){
		$('.date-menu-li').eq(0).children('.no-choose').children('.myWeek').text("今日");
		v=$('.date-menu-li:eq('+ix+')').offset().left;
		$scope.nowMap=($('.date-menu-li:eq('+ix+')').text()).substr(0,2);
		$scope.nowDay=$('.date-menu-li:eq('+ix+')').attr('nowDay');
		if(ix==0)
			angular.element('.date-menu-active').css('left','0');
		else
			angular.element('.date-menu-active').css('left',(v-20)+'px');
		if($scope.courseList)
			$scope.nowCourseList=$scope.courseList[time];
	}
	//获取当天时间
  ...
  //加载
	$scope.load=function(){
		ajaxService.ajax('getPlanCourseList',{
			type      : 1,    //1公开课，2 小团课, 3私教课。不传所有
			province	:	window.userLoc.province,
			city		  :	window.userLoc.city,
			longitude	:	window.userLoc.longitude,
			latitude	:	window.userLoc.latitude,
...
//公共课详情
app.controller("userComCourseDtlCtrl", ["$scope","$ionicLoading","ajaxService","$popup","$location","$stateParams","$ionicPopup","$ionicSlideBoxDelegate","$timeout","infoService",
				   			function ($scope,$ionicLoading,ajaxService,$popup,$location,$stateParams,$ionicPopup,$ionicSlideBoxDelegate,$timeout,infoService) {

	$scope.courseID = '';
	$scope.courseComment = [];
	$scope.courseComment.page_no = 1;
	...
	$scope.load=function(){
		ajaxService.ajax('getPlanInfo',{
			...
			if(json.code==200){
				$scope.comCourseDtl=json.data;
				//ionic轮播图无限播放
				// $ionicSlideBoxDelegate.update();
				// $ionicSlideBoxDelegate.$getByHandle('slideimgs').loop(true);
				$scope.comCourseDtl.coach.intro = $scope.comCourseDtl.coach.intro.replace(/<\/?[^>]*>/g, '');//过滤html所有标签
				$scope.comCourseDtl.course.intro = $scope.comCourseDtl.course.intro.replace(/<\/?[^>]*>/g, '');//过滤html所有标签
				$scope.progressBar = $scope.comCourseDtl.user_count/$scope.comCourseDtl.max_count*100+'%';//进度条百分比计算

				$scope.courseID = $scope.comCourseDtl.course_id;
				$scope.loadComment();
			}
		})
	}
		...
		if(!window.member){
          ...
          $timeout(function() {
            ...
            $scope.go('您还未购买，或是课程已过期了呢',2,null);
						...
						// $popup.alert('尊敬的用户','',json.message);
						var myPopup = $ionicPopup.show({
	            title: json.message, // String. 弹窗的标题。
	          });
	          $timeout(function() {
	            myPopup.close(); // 3秒后关闭弹窗
	          }, 1500);
	...
	$scope.goRules=function(id){
		$location.path('users/appt-com-rules/'+id);
	}

	$scope.loadComment=function(){
		ajaxService.ajax('getcommentList',{
			type      : 1,                  //评价类型（1为课程，2为私教）
			id 		    :	$scope.courseID,
			page_no   : $scope.courseComment.page_no,
			page_size : 5,
		},function(json){
			if(json.code==200){
				$scope.commentList=json.data;
				for(var i=0;i<$scope.commentList.list.length;i++){
					$scope.courseComment.push($scope.commentList.list[i]);
				}
			}
		})
	}
	...
	$scope.moreComment=function(){
		$scope.courseComment.page_no = $scope.courseComment.page_no+1;
		$scope.loadComment();
	}
...
//公共课约课规则
app.controller("userApptComRulesCtrl", ["$scope","$ionicLoading","ajaxService","$popup","$location","$stateParams","$ionicPopup","$ionicSlideBoxDelegate","$timeout","infoService",
				   			function ($scope,$ionicLoading,ajaxService,$popup,$location,$stateParams,$ionicPopup,$ionicSlideBoxDelegate,$timeout,infoService) {

	$scope.goClose=function(){
		$location.path('users/comCourse-dtl/'+ $stateParams.id);
	}
...
//精品团课排课列表
app.controller("userGroCoursePlanCtrl", ["$scope","$ionicLoading","ajaxService","formatDate","$filter","$timeout","$location",
				   			 function ($scope,$ionicLoading,ajaxService,formatDate,$filter,$timeout,$location) {
  ...
  //加载
	...
	$scope.go=function(id){
		$location.path('users/groCourse-dtl/'+id)
	}
	//页面刚加载			   
	$scope.$on('$ionicView.beforeEnter',function(){
...
//精品团课详情
app.controller("userGroCourseDtlCtrl", ["$scope","$ionicLoading","ajaxService","$popup","$location","$stateParams","$ionicPopup","$ionicSlideBoxDelegate","$timeout","infoService","$ionicModal",
				   			function ($scope,$ionicLoading,ajaxService,$popup,$location,$stateParams,$ionicPopup,$ionicSlideBoxDelegate,$timeout,infoService,$ionicModal) {
	...
	$scope.load=function(){
		ajaxService.ajax('getPlanInfo',{
			...
			if(json.code==200){
				$scope.groCourseDtl=json.data;
				console.log($scope.groCourseDtl);
				//ionic轮播图无限播放
				// $ionicSlideBoxDelegate.update();
				// $ionicSlideBoxDelegate.$getByHandle('slideimgs').loop(true);
				$scope.groCourseDtl.coach.intro = $scope.groCourseDtl.coach.intro.replace(/<\/?[^>]*>/g, '');//过滤html所有标签
				$scope.groCourseDtl.course.intro = $scope.groCourseDtl.course.intro.replace(/<\/?[^>]*>/g, '');//过滤html所有标签
				$scope.progressBar = $scope.groCourseDtl.user_count/$scope.groCourseDtl.max_count*100+'%';//进度条百分比计算

				$scope.courseID = $scope.groCourseDtl.course_id;
				$scope.starsNum = [];
				for(var i=0;i<$scope.groCourseDtl.coach.score;i++){
					$scope.starsNum.push(i);
				}
				$scope.loadComment();
		...
		ajaxService.ajax('getcommentList',{
			type          :    1,                           					//评价类型（1为课程，2为私教）
			id            :    $scope.groCourseDtl.course.course_id,  //课程传course_id,私教传user_id
			page_no       :    1,                 										//当前页
			// page_size     :    100,     											 				//每页显示条数
		},function(json){
			if(json.code==200){
				$scope.groCourseAssessment=json.data;
			}
		})
	}
	//约课
  ...
  $scope.$on('$destroy', function() {
			  ...
			  if(res) {
			    if(type==1){
			      $location.path('users/card-list')
						...
						// $location.path('users/groCourse-buy/'+$stateParams.planId);
						$scope.modal.show();
						// $("#buyGroCourse-model button.first").css({"border":"0.013rem solid #ff9d35","color":"#ff9d35"});
	...
	// $scope.chooseNum=function(num){
	// 	console.log(num);
	// 	if(num == 0){
	// 		$("#buyGroCourse-model button.first").css({"border":"0.013rem solid rgba(0,0,0,0.12)","color":"#9b9b9b"});
	// 		$("#buyGroCourse-model button.second").css({"border":"0.013rem solid #ff9d35","color":"#ff9d35"});
	// 	}else{
	// 		$("#buyGroCourse-model button.first").css({"border":"0.013rem solid #ff9d35","color":"#ff9d35"});
	// 		$("#buyGroCourse-model button.second").css({"border":"0.013rem solid rgba(0,0,0,0.12)","color":"#9b9b9b"});
	// 	}
	// }
...
//私教列表
app.controller("userCoachListCtrl", ["$scope","$ionicLoading","ajaxService","$timeout","$location",
				   	   function ($scope,$ionicLoading,ajaxService,$timeout,$location) {
	...
	$scope.load=function(){
		...
		ajaxService.ajax('getCoachList',{
			...
			if(json.code==200){
				var sum=[];
				for(var i=0;i<json.data.length;i++){
					if(json.data[i].type==2 && json.data[i].price)//判断type://1全职 2兼职，price价格存在的时候
						sum.push(json.data[i]);
					if(i+1==json.data.length)
						$scope.coachList=sum
				}
	...
	$scope.sort=function(type){
		...
		if(type==1){
			$scope.list.order=''
			$('.coach-screen-li').css("color","#323232");
			$('.coach-screen-li[name = '+1+']').css("color","#ff9d35");
		}
			
		//销量排序
		if(type==2){
			$('.coach-screen-li').css("color","#323232");
			$('.coach-screen-li[name = '+2+']').css("color","#ff9d35");
			if($scope.list.order=='' || $scope.list.order=='price asc' || $scope.list.order=='price desc'){
				$scope.list.order='buy_num asc'
			}else{
		...
		if(type==3){
			$('.coach-screen-li').css("color","#323232");
			$('.coach-screen-li[name = '+3+']').css("color","#ff9d35");
			if($scope.list.order=='' || $scope.list.order=='buy_num asc' || $scope.list.order=='buy_num desc'){
				$scope.list.order='price asc'
			}else{
...
//私教详情
app.controller("userCoachDtlCtrl", ["$scope","$ionicLoading","ajaxService","$stateParams","$filter","formatDate","infoService","$location","$ionicPopup",
				   	  function ($scope,$ionicLoading,ajaxService,$stateParams,$filter,formatDate,infoService,$location,$ionicPopup) {
...
//精品团课列表
app.controller("userGroCourseListCtrl", ["$scope","$ionicLoading","ajaxService","formatDate","$filter","$timeout","$location",'infoService','$state',
				   			function ($scope,$ionicLoading,ajaxService,formatDate,$filter,$timeout,$location,infoService,$state) {
  //加载
	$scope.load=function(){
		ajaxService.ajax('getCourseList',{
    ...
    $state.go('users.groCourse-info', {item:item});
	}
...
//精品团课详情
app.controller("userGroCourseInfoCtrl", ["$scope","$ionicLoading","ajaxService","$popup","$location","$stateParams","$ionicPopup","$ionicSlideBoxDelegate","$timeout","infoService","$ionicModal","$state",
				   			function ($scope,$ionicLoading,ajaxService,$popup,$location,$stateParams,$ionicPopup,$ionicSlideBoxDelegate,$timeout,infoService,$ionicModal,$state) {

	$scope.groCourseDtl = $stateParams.item;
	if($stateParams.item){
		$scope.groCourseDtl.coach.intro = $scope.groCourseDtl.coach.intro.replace(/<\/?[^>]*>/g, '');//过滤html所有标签
		$scope.groCourseDtl.intro = $scope.groCourseDtl.intro.replace(/<\/?[^>]*>/g, '');//过滤html所有标签
	}
	if($stateParams.item != null){
		$scope.loadComments = function(){
			ajaxService.ajax('getcommentList',{
				type      :   1,                               //评价类型（1为课程，2为私教）
				id        :   $stateParams.item.course_id,     //课程传course_id,私教传user_id
				page_no   :   1,                               //当前页
				page_size :   10,                              //每页显示条数
			},function(json){
				if(json.code==200){
					$scope.courseComments=json.data;
				}
			})
		}
		$scope.loadComments();
	...
	$scope.loadShop = function(){
		ajaxService.ajax('shopDetail',{
			...
			if(json.code==200){
				$scope.shopDtl=json.data;
			}
		})
	}
	$scope.loadShop();

	if($stateParams.item == null || $stateParams.item == 'null'){
		$location.path('users/groCourse-list');
	}
  ...
  $scope.$on('$destroy', function() {
	...
	//创建订单
	$scope.creacteOrder=function(item){
		$scope.modal.hide();
		ajaxService.ajax('createorder',{
			type            :    2,                           //购买类型，1会员卡，2小团课，3私教课，4储值，5取消旷课，6添加会员卡子成员
			seller_id       :    '',                          //会员卡就是会籍id
			product_num     :    $scope.groCourseDtl.min_buy, //购买数量,会员卡传1，购课传相应数量
			product_id      :    item.course_id,              //商品ID，会员卡传card_id,课程传course_id
			shop_id         :    window.shop.id,              //会员卡订单必传，算作哪个店的业绩
			// origin_price    :    '',                          //储值才传。储值多少金豆。
		},function(json){
			if(json.code==200){
				// $location.path('users/groCourse-order/'+json.data.order_id);
				$state.go('users.groCourse-order', {item:item,orderId:json.data.order_id});
			}
		})
	}
...
//精品团课订单详情
app.controller("userGroCourseOrderCtrl", ["$scope","$ionicLoading","ajaxService","$popup","$location","$stateParams","$ionicPopup","$ionicSlideBoxDelegate","$timeout","infoService","$state","$ionicModal",
				   			function ($scope,$ionicLoading,ajaxService,$popup,$location,$stateParams,$ionicPopup,$ionicSlideBoxDelegate,$timeout,infoService,$state,$ionicModal) {

	if($stateParams.item == null || $stateParams.item == 'null'){
		$location.path('users/groCourse-list');
	}

	$scope.loadShopDtl = function(){
		if(window.shop.id == ''){
			window.shop.id == 2;
		}
		ajaxService.ajax('shopDetail',{
			shop_id    :     window.shop.id,      //店铺id
		},function(json){
			if(json.code==200){
				$scope.shopDtl=json.data;
			}
	...
	$scope.loadOrderDtl = function(){
		ajaxService.ajax('orderDetail',{
			order_id     :     $stateParams.orderId,         //订单id
		},function(json){
			if(json.code==200){
				$scope.orderDtl=json.data;
				console.log($scope.orderDtl);
				console.log($stateParams.item);
				$scope.orderTxt = $stateParams.item;
				if($scope.shopName){
				...
				var ipos;
				ipos = $scope.orderDtl.product_info.indexOf("*");
				$scope.orderDtl.product_info=$scope.orderDtl.product_info.substring(0,ipos); 
			}
		})
	}
	$scope.loadOrderDtl();
  ...
  $scope.$on('$destroy', function() {
	...
	$scope.getCoupon=function(){
		$scope.modal.show();
		$scope.couponType = 1;
		$(".coupon-btn1").css({"color":"#ff9d35","border-bottom":"0.027rem solid #ff9d35"});
		$(".coupon-btn2").css({"color":"#323232","border-bottom":"0.027rem solid #fff"});
		$(".coupon_bg").attr("src", "./static/mobilev2/images/coupon_bg1.png"); 
	}
	//切换优惠券
	$scope.loadCoupon=function(item){
		console.log(item);
		$scope.couponType = item;
		if(item==1){
			$(".coupon-btn2").css({"color":"#323232","border-bottom":"0.027rem solid #fff"}); 
			$(".coupon-btn1").css({"color":"#ff9d35","border-bottom":"0.027rem solid #ff9d35"}); 
		}
		if(item==0){
			$(".coupon-btn1").css({"color":"#323232","border-bottom":"0.027rem solid #fff"});
			$(".coupon-btn2").css({"color":"#ff9d35","border-bottom":"0.027rem solid #ff9d35"});  
		}
	}

	$scope.chooseCoupon=function(id){
		console.log(id);
		$(".coupon_bg").attr("src", "./static/mobilev2/images/coupon_bg1.png");  
		$('.coupon_bg').attr("src", "./static/mobilev2/images/coupon_bg2.png");  
	}
